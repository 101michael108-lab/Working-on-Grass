  ...
  return (
    <div className="bg-background">
      ...
      <section className="bg-muted/20 border-b-2 border-border">
        <div className="container py-12 md:py-20 grid lg:grid-cols-12 gap-12 items-center">
                ...
                <div className="bg-background border-2 border-border p-8 rounded-lg max-w-md shadow-sm">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-4">
                            <div className="flex items-center border-2 border-border rounded h-12">
                                <Button variant="ghost" size="icon" className="h-full w-12" onClick={() => handleQuantityChange(-1)}><Minus className="h-4 w-4" /></Button>
                                <Input type="number" className="w-16 text-center border-0 shadow-none focus-visible:ring-0 text-lg font-bold" value={quantity} onChange={(e) => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} />
                            ...
                            <TableBody>
                                {product.specifications.map((spec, index) => (
                                    <TableRow key={index} className={index % 2 === 0 ? 'bg-muted/30' : ''}>
                                        <TableCell className="font-bold text-primary/70 py-4 pl-6 uppercase tracking-wider text-[10px] w-1/3">{spec.feature}</TableCell>
                                        <TableCell className="py-4 pr-6 text-foreground font-body font-medium">{spec.description}</TableCell>
                                    </TableRow>
                                ))}
                            </TableBody>
                        </Table>
