...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Card className="flex flex-col group overflow-hidden transition-all hover:border-primary/50 h-full border-2 relative">
      {isOutOfStock && (
        <Badge variant="destructive" className="absolute top-4 right-4 z-10 font-bold shadow-lg">
      ...
      <Link href={`/shop/${product.id}`} className="block relative aspect-square bg-muted/50 overflow-hidden border-b-2">
        <Image
          ...
          className={`object-cover h-full w-full group-hover:scale-105 transition-transform duration-500 ${isOutOfStock ? 'grayscale opacity-60' : ''}`}
        />
      </Link>
      ...
      <CardFooter className="p-5 mt-auto border-t-2 bg-muted/20">
        <div className="flex justify-between items-center w-full">
            ...
            >
                <ShoppingCart className="mr-2 h-4 w-4" /> {isOutOfStock ? 'Sold Out' : 'Add to Cart'}
            </Button>
        </div>
      </CardFooter>
    </Card>
